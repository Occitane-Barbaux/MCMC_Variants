...
def Full_Run_Analysis_simulated(dt_string,pathOut,model_name,prior_mean,prior_cov,true_theta=np.array([-0.3, 1.3,  0.5 ,  0.02, -0.25]),init=np.array([0,0,0,0,0]),tran_scale_G=np.array([1,1,1,1,1]),n_mcmc_drawn=10000,n_sim=37,n_features=5,TransitionAdapt=True,Within_Gibbs=True,show=True,initTrans=0.01,epsilon=0.01):
    ...
    if Within_Gibbs:
    	draw, accept=MCMC_MH_W_Gibbs(prior_law,sdlaw,tran_scale_G,init,n_mcmc_drawn,n_features,TransitionAdapt)
    else:
    ...
    Summary_run_table(pathOut,model_name,draw,accept,true_theta,MLE_theta,q_l=0.05,q_h=0.95,coef=['loc0' ,'loc1', 'scale0', 'scale1' ,'shape0'],show=show)
    Para_Runs(pathOut,draw,true_theta,MLE_theta,prior_mean,model_name,prior_law,show=show)
    return draw,accept
...
def Full_Run_Analysis_Data(dt_string,
	...
	Summary_run_table(pathOut,model_name,draw,accept,true_theta,MLE_theta,q_l=0.05,q_h=0.95,coef=['loc0' ,'loc1', 'scale0', 'scale1' ,'shape0'],show=show,Simulated=False)
	true_theta=[]
	Para_Runs(pathOut,draw,true_theta,MLE_theta,prior_mean,model_name,prior_law,show=show)
	return draw,accept
