...
def constrain_MCMC_MH_law_all( clim , Yo,tran_scale_G=np.array([1,1,1,1,1]),init=np.array([0,0,0,0,0]),n_features=5,TransitionAdapt=True,initTrans=0.01,epsilon=0.01,n_sortie=100,burn_in=1000,  verbose=True , **kwargs ):##{{{
	clim2 = clim.copy()
	n_ns_params = clim.ns_law.n_ns_params
	sample    = clim.sample
	n_sample=len(sample)
...
def constrain_MCMC_MHWG_law_all( clim , Yo,tran_scale_G=np.array([1,1,1,1,1]),init=np.array([0,0,0,0,0]),n_features=5,TransitionAdapt=True,initTrans=0.01,epsilon=0.01,n_sortie=100,burn_in=1000,  verbose=True , **kwargs ):##{{{
	clim2 = clim.copy()
	n_ns_params = clim.ns_law.n_ns_params
	sample    = clim.sample
	n_sample=len(sample)
