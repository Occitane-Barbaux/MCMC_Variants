...
def MCMC_MH_W_Gibbs_old(prior_law,sdlaw,tran_scale_G=np.array([1,1,1,1,1]),prior_mean=np.array([0,0,0,0,0]),Cov_dis_full=np.array([0,0,0,0,0]),init=np.array([0,0,0,0,0]),n_mcmc_drawn=10000,n_features=5,TransitionAdapt=True):
	#False, used the conditional marginal prior and not the actual marginal prior
	draw = np.zeros( (n_mcmc_drawn,n_features) )
...
def MCMC_MH_W_Gibbs(prior_law, #Full multivariate prior
	sdlaw, # Obs likelihood
	tran_scale_G=np.array([1,1,1,1,1]), #If no Adaptation, Transition used
	init=np.array([0,0,0,0,0]),
	n_mcmc_drawn=10000,
	n_features=5,
	TransitionAdapt=True):
	
	#use the actual prior for multivariate gaussian directly
	draw = np.zeros( (n_mcmc_drawn,n_features) )
	accept = np.zeros((n_mcmc_drawn,n_features) , dtype = np.bool )
	#prior_current=np.zeros(n_features)
	draw[0,:]     = init
